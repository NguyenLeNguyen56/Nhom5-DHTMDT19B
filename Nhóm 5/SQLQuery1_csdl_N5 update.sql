...
--I.Truy vấn Join
--Câu hỏi 1: Liệt kê mã giáo viên, họ tên, và tổng số tiết giảng dạy thực tế, chỉ hiển thị những giáo viên đã dạy từ 6 tiết trở lên trong tất cả các lịch dạy.
SELECT GV.MAGV, 
       GV.HOTEN, 
...
--1. Liệt kê giáo viên và số môn học họ đang phụ trách
SELECT GV.MAGV, GV.HOTEN, COUNT(MH.MAMH) AS SoMonPhuTrach
FROM GIAO_VIEN GV
LEFT JOIN MON_HOC MH ON GV.MAGV = MH.MAGV
GROUP BY GV.MAGV, GV.HOTEN;

--2. Lớp thuộc khoa CNTT và có sĩ số > 40
SELECT LH.MALOP, LH.TENLOP, LH.SISO
FROM LOP_HOC LH
JOIN KHOA K ON LH.MAKHOA = K.MAKHOA
WHERE K.TENKHOA = N'Công nghệ thông tin' AND LH.SISO > 40;
--3. Số lần môn học được giảng dạy
SELECT MH.MAMH, MH.TENMH, COUNT(*) AS SoLanDay
FROM MON_HOC MH
JOIN LICH_DAY LD ON MH.MAMH = LD.MAMH
GROUP BY MH.MAMH, MH.TENMH;
--4. Giáo viên dạy từ 2 lớp trở lên
SELECT MAGV, COUNT(DISTINCT MALOP) AS SoLopDay
FROM LICH_DAY
GROUP BY MAGV
HAVING COUNT(DISTINCT MALOP) >= 2;
--5. Phòng học được dùng hơn 1 buổi
SELECT PH.SOPHONG, PH.CHUCNANG, COUNT(*) AS SoBuoiDay
FROM PHONG_HOC PH
JOIN LICH_DAY LD ON PH.SOPHONG = LD.SOPHONG
GROUP BY PH.SOPHONG, PH.CHUCNANG
HAVING COUNT(*) > 1;
